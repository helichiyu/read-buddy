...
> 本文档基于 v0.1.0 实际代码梳理（2026-06-23）。如代码与本文描述不符，以代码为准并更新本文档。
...
| 模块 | 职责 | 关键内容 |
|---|---|---|
| `app.py` | **路由层 + 对话编排** | 全部 HTTP 路由；`/api/chat` 承担工具调用循环（最多 5 轮）；导出/导入用 tkinter 弹原生对话框 |
| `ai_service.py` | **AI 服务层** | `build_system_prompt()` 动态拼接上下文；`AIService` 封装 AsyncOpenAI；`get_ai_service()` 从配置构造实例 |
| `database.py` | **数据访问层** | async 函数式 API，覆盖 books/ratings/messages/settings/profile 的增删改查与导入导出 |
| `book_service.py` | **外部书籍查询** | `search()`：豆瓣优先、Google Books 兜底，返回封面/简介/ISBN/分类 |
| `preferences.py` | **偏好记忆** | 以 Markdown 文件存储用户偏好，提供读/追加 |
| `models.py` | 数据模型 | Pydantic 模型定义（详见「可优化点」） |
...
- **`app.py` 是唯一枢纽**：前端只通过 HTTP 与后端交互；所有外部入口收敛到路由层。
...
│   ├── book_service.py    # 豆瓣 + Google Books 查询
...
| 打包双路径模式（`sys.frozen` 区分） | 兼容开发态与 PyInstaller 冻结态，数据写可写目录、资源读只读 `_MEIPASS` | 路径判断在多处重复 |

## 8. 可优化点

> 仅作建议，**不擅自重构**，是否执行由用户决定。

1. **`models.py` 定义了但运行时未使用**：`app.py` 用裸 `dict = Body(...)`、`database.py` 返回 dict，Pydantic 模型未参与校验/序列化。要么用起来（路由参数与返回模型化），要么删除以免文档误导。
2. **`database.py` 无连接复用**：每次操作都 `connect` + `close`，高频/并发时有开销。可考虑应用级单连接或连接池。
3. **`app.py` 的 `/api/chat` 编排较重**（工具循环、上下文构建耦合在路由内）。规模尚可控；若工具与规则继续增长，可抽 `chat_orchestrator`。
4. **System Prompt 随数据膨胀**：把全部已读/待读/拒绝列表塞进 prompt，书籍上百后 token 成本与上下文窗口承压。可考虑摘要/截断/检索式注入。
5. **部分状态约束靠 Prompt 软约束**：如「同一本最多推荐 3 次」「不重复推荐已读」。可靠性依赖模型遵循，可在代码侧加兜底校验。
6. **导出/导入存在冗余实现**：后端有 4 个接口（`/api/export`、`/api/import`、`/api/export-to-file`、`/api/import-from-file`），前端按钮只用后两个（原生对话框版）；`api.js` 中的 `exportData`/`importData` 与 `/api/export`、`/api/import` 实为未调用代码，建议清理。
7. **`book_service` 无缓存、详情靠正则抓 HTML**：重复书会重复查；豆瓣详情页用正则提取 `og:description` 较脆弱，豆瓣改版即失效。
8. **推荐阶段无封面/详情（懒加载代价）**：书籍详情仅在 `accept_book`/`rate_book` 时查询，推荐卡片在用户接受前缺少封面与简介，体验单薄。可考虑推荐时轻量预取（如仅取封面），或推荐即落库时异步补全——需权衡额外 API 调用与推荐即时性。
9. **多处 `except Exception: pass` 静默吞错**（`book_service`、工具内）：桌面应用可接受，但排障困难，可加日志。
10. **缺少自动化测试**：与「目标驱动执行」精神有差距，关键链路（工具分发、状态机、导入导出）适合补单测。
11. **`sys.frozen` 路径判断散落 4 处**（`main.py`/`app.py`/`database.py`/`preferences.py`），可抽公共路径工具统一。
