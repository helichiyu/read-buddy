...
![](https://img.shields.io/badge/Python-3.14-blue) ![](https://img.shields.io/badge/Version-0.1.2-green) ![](https://img.shields.io/badge/Platform-Windows-informational)
...
👉 [**下载 ReadBuddy_Setup_v0.1.2.zip**](https://github.com/helichiyu/read-buddy/releases/download/v0.1.2/ReadBuddy_Setup_v0.1.2.zip)，解压后双击安装即可。
...
| `GET` | `/api/book-cover` | 封面代理（绕豆瓣防盗链） |
...
- **修复打包版无法保存数据**：默认安装到 Program Files 时普通用户无写权限，阅读记录与评价存不下来、关掉重开就丢失；现改存到系统标准位置 `%APPDATA%\ReadBuddy`，数据持久保留
- **修复书籍封面不显示**：豆瓣图床防盗链导致封面图片加载失败；新增后端封面代理接口，封面恢复正常显示
- 同步卸载清理路径到新数据目录；清理误入版本库的安装程序产物

> **从旧版本升级**：直接运行新版安装包覆盖安装即可，无需先卸载。但本版数据存储位置由「安装目录\data」改为 `%APPDATA%\ReadBuddy`，**旧版数据不会自动迁移**。若旧版有阅读记录想保留：升级前先用旧版「导出存档」导出，升级后再用「导入存档」导入。

### v0.1.1 (2026-06-25)

内部重构与质量提升，无用户可见功能变化：

- 数据库改为应用级单连接复用，减少频繁建连开销
- 书籍信息查询增加内存缓存，避免对同一本书重复请求豆瓣 / Google Books
- 对话编排逻辑抽离为独立模块（`chat_orchestrator`），核心路由瘦身
- 统一开发态与打包态的路径判断（新增 `paths` 工具模块）
- 静默异常改为日志记录，便于排查问题
- 引入 pytest 单元测试，覆盖工具注册、状态机、导出导入等关键链路
- 清理未使用代码与冗余的导出/导入接口
